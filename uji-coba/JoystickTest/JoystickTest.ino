/*
 * JoystickTest.ino - RC-HIMA joystick test (tanpa LoRa / OLED)
 *
 * Mapping joystick (MODE 2):
 ...
 *
 * ROLL/PITCH/YAW : 0..255, center = 128
 * THROTTLE       : 0..255, bawah = 0, atas = 255
 *
 * Output serial 115200, 20 Hz:
...
const float OUTPUT_PERIOD_MS      = 50.0f; // 20 Hz
...
void setup()
{
  ...
  Serial.println("=== JOYSTICK TEST v2 ===");
  if (SWAP_LEFT_X_Y)  Serial.println("NOTE: SWAP_LEFT_X_Y=true  (ROLL/THROTTLE ditukar).");
  if (SWAP_RIGHT_X_Y) Serial.println("NOTE: SWAP_RIGHT_X_Y=true (YAW/PITCH ditukar).");
...
void loop()
{
  ...
  for (int ch = 0; ch < N_CH; ch++) {
    ...
    raw[ch] = v;                          // nilai ADC (sudah invert), 0-4095
  ...
  Serial.print("JOY LX:"); Serial.print(raw[0]);
  Serial.print(" LY:");    Serial.print(raw[1]);
  Serial.print(" RX:");    Serial.print(raw[2]);
  Serial.print(" RY:");    Serial.print(raw[3]);
  Serial.print(" R:");     Serial.print(cmd[0]);
  Serial.print(" T:");     Serial.print(cmd[1]);
  Serial.print(" Y:");     Serial.print(cmd[2]);
  Serial.print(" P:");     Serial.print(cmd[3]);
  Serial.println();
