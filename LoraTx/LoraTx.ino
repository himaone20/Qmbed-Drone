...
// nilai yang disuntik. Urutan field paket uplink (R/T/Y/P) TIDAK berubah.
//  - guiRoll/guiYaw/guiPitch = 128 (netral) -> drone auto-level 0 derajat
//  - guiThrottle = 0..255 dari GUI
...
// Saat override aktif, percepat siklus ping-pong ke ~20 Hz untuk tuning real-time.
const unsigned long CYCLE_PERIOD_OVERRIDE_MS = 45;  // ~22 Hz
const unsigned long REPLY_TIMEOUT_OVERRIDE_MS = 35;
...
// Joystick bertipe pegas (self-centering): semua channel menggunakan centerAxisToCmd
// agar posisi netral (lepas stik) selalu tepat 128 dengan deadzone.
...
void renderDashboard(uint8_t rawR, uint8_t rawT, uint8_t rawY, uint8_t rawP,
                     float rollDeg, uint16_t throttlePwm, float yawRateDps, float pitchDeg,
  ...
  // Garis pemisah status bar horizontal
...
void loop()
{
  ...
  while (Serial.available()) {
    ...
    if (input.equalsIgnoreCase("ARM")) {
    ...
    } else if (input.startsWith("STICK ")) {
      // Mode Tes PID: override stik dari GUI. Format: STICK <throttle0..255>
      // Roll/Yaw/Pitch otomatis 128 (netral) -> drone auto-level 0 derajat.
      ...
      if (t >= 0 && t <= 255) {
        guiThrottle = t;
        guiOverride = true;
        Serial.print("[STICK] Override ON T=");
        Serial.println(guiThrottle);
      } else {
        Serial.println("[STICK] ERR throttle (0..255)");
      }
    } else if (input.startsWith("STICKSTOP")) {
      guiOverride = false;
      Serial.println("[STICK] Override OFF (kembali ke joystick hardware)");
    } else if (input.equals("STICKRESET")) {
      guiThrottle = 0;
      guiOverride  = false;
      ...
      if (count >= 12) {
        ...
        if (cfg.maxYawRate >= 30.0f && cfg.maxYawRate <= 300.0f) {
          remoteMaxYawRateDps = cfg.maxYawRate;
        }
        if (count >= 15) {
          if (cfg.escMinPwm >= 900.0f && cfg.escMinPwm <= 1400.0f) {
            throttleMinPwm = (uint16_t)cfg.escMinPwm;
          }
          if (cfg.escArmSpinPwm >= 1000.0f && cfg.escArmSpinPwm <= 1600.0f) {
            throttleIdlePwm = (uint16_t)cfg.escArmSpinPwm;
          }
          ...
          if (cfg.escArmSpinPwm >= 1100.0f && cfg.escArmSpinPwm <= 2200.0f) {
            throttleMaxPwm = (uint16_t)cfg.escArmSpinPwm;
            cfg.escArmSpinPwm = (float)throttleIdlePwm;
          }
        }
        LoRa.beginPacket();
  ...
  /* ---------- [1] Baca Joystick / Override GUI & Konversi ke Sudut ---------- */
  int cmd[N_CH];
  if (guiOverride) {
  ...
  int rDelta = dispR - 128;
  if (abs(rDelta) > STICK_DEADBAND) {
    targetRollDeg = ((float)rDelta / 127.0f) * remoteMaxAngleDeg;
    targetRollDeg = constrain(targetRollDeg, -remoteMaxAngleDeg, remoteMaxAngleDeg);
  }

  // 2. Pitch: dispP 128 -> 0.0°, 255 (Dorong Maju) -> -25.0° (Nose Down / Maju), 0 (Tarik Mundur) -> +25.0° (Nose Up / Mundur)
  float targetPitchDeg = 0.0f;
  int pDelta = dispP - 128;
  if (abs(pDelta) > STICK_DEADBAND) {
    targetPitchDeg = -((float)pDelta / 127.0f) * remoteMaxAngleDeg;
    targetPitchDeg = constrain(targetPitchDeg, -remoteMaxAngleDeg, remoteMaxAngleDeg);
  }

  // 3. Yaw: dispY 128 -> 0.0°/s, 255 -> +150.0°/s (CW/Kanan), 0 -> -150.0°/s (CCW/Kiri)
  float targetYawRateDps = 0.0f;
  int yDelta = dispY - 128;
  if (abs(yDelta) > STICK_DEADBAND) {
    targetYawRateDps = ((float)yDelta / 127.0f) * remoteMaxYawRateDps;
  ...
  up.targetRoll     = (int16_t)(targetRollDeg * 100.0f);
  up.targetPitch    = (int16_t)(targetPitchDeg * 100.0f);
  up.targetYaw      = (int16_t)(targetYawRateDps * 100.0f);
  up.targetThrottle = targetThrottlePwm;
  ...
  unsigned long replyTimeout = guiOverride ? REPLY_TIMEOUT_OVERRIDE_MS : REPLY_TIMEOUT_MS;
  while (!gotReply && (millis() - waitStart < replyTimeout))
  {
    ...
    if (packetSize >= (int)sizeof(DownlinkPacket))
    {
      ...
      if (down.magic == DOWNLINK_MAGIC)
      {
        ...
        // Decode & print status flags dari drone
        //   G  = gyroCalibValid (1=OK, 0=FAIL)
        //   B  = bmiOK, P = bmpOK, V = vbatOK
        //   BS = battStage (0=OK 1=WARN 2=LIMIT 3=CRITICAL)
        //   FS = failsafeStage (0=OK 2=DESCENT 3=LAND)
        Serial.print("[FLAGS] G:"); Serial.print((down.flags & 0x01) ? 1 : 0);
        Serial.print(" B:");  Serial.print((down.flags & 0x02) ? 1 : 0);
        Serial.print(" P:");  Serial.print((down.flags & 0x04) ? 1 : 0);
        Serial.print(" V:");  Serial.print((down.flags & 0x08) ? 1 : 0);
        Serial.print(" BS:"); Serial.print((down.flags >> 4) & 0x03);
        Serial.print(" FS:"); Serial.println((down.flags >> 6) & 0x03);
      }
    }
  ...
  if (oledOK) {
    renderDashboard((uint8_t)dispR, (uint8_t)dispT, (uint8_t)dispY, (uint8_t)dispP,
                    targetRollDeg, targetThrottlePwm, targetYawRateDps, targetPitchDeg,
                    avgRssi, lossPct, sigStatus, consecutiveMiss);
  }

  /* ---------- [5] Jaga total periode siklus (normal ~7Hz, override ~20Hz) ----- */
  unsigned long elapsed = millis() - cycleStart;
  unsigned long cyclePeriod = guiOverride ? CYCLE_PERIOD_OVERRIDE_MS : CYCLE_PERIOD_MS;
  if (elapsed < cyclePeriod) {
    delay(cyclePeriod - elapsed);
  }
}
