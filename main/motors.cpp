...
uint8_t gMotorFaultActive[4] = {0, 0, 0, 0};
float   gMotorFaultPercent[4] = {100.0f, 100.0f, 100.0f, 100.0f};
bool    gEnableFtc = true;

void setMotorFault(int motorIndex, uint8_t active, float percent)
{
  if (motorIndex >= 0 && motorIndex < 4) {
    gMotorFaultActive[motorIndex] = active ? 1 : 0;
    gMotorFaultPercent[motorIndex] = constrain(percent, 0.0f, 100.0f);
  }
}
...
void writeMotorMix(float basePwm, float uRoll, float uPitch, float uYaw)
{
  ...
  // ── Fault Tolerant Control (FTC) Re-allocation ──
  // Jika FTC aktif dan terjadi kegagalan/degradasi pada 1 motor,
  // relaksasikan kendali yaw (agar motor sehat tidak jenuh) dan seimbangkan torsi Roll/Pitch
  if (gEnableFtc) {
    int faultCount = 0;
    int failedIdx = -1;
    for (int i = 0; i < 4; i++) {
      if (gMotorFaultActive[i] && gMotorFaultPercent[i] > 5.0f) {
        faultCount++;
        failedIdx = i;
      }
    }
    if (faultCount == 1) {
      uYaw = 0.0f; // Relaksasi yaw: toleransi perputaran perlahan demi menjaga drone tetap level
      float loss = constrain(gMotorFaultPercent[failedIdx], 0.0f, 100.0f) / 100.0f;
      float eff = 1.0f - loss;

      // Re-alokasi torsi: turunkan motor diagonal & naikkan pasangan ortogonal
      if (failedIdx == 0) { // M1 (FL) gagal
        float lostThrust = (basePwm + uRoll + uPitch - (float)gEscMinPwm) * loss;
        float boost = lostThrust * 0.52f;
        uRoll += boost * 0.5f;
        uPitch -= boost * 0.5f;
      } else if (failedIdx == 1) { // M2 (FR) gagal
        float lostThrust = (basePwm - uRoll + uPitch - (float)gEscMinPwm) * loss;
        float boost = lostThrust * 0.52f;
        uRoll -= boost * 0.5f;
        uPitch -= boost * 0.5f;
      } else if (failedIdx == 2) { // M3 (BR) gagal
        float lostThrust = (basePwm - uRoll - uPitch - (float)gEscMinPwm) * loss;
        float boost = lostThrust * 0.52f;
        uRoll -= boost * 0.5f;
        uPitch += boost * 0.5f;
      } else if (failedIdx == 3) { // M4 (BL) gagal
        float lostThrust = (basePwm + uRoll - uPitch - (float)gEscMinPwm) * loss;
        float boost = lostThrust * 0.52f;
        uRoll += boost * 0.5f;
        uPitch += boost * 0.5f;
  ...
  // ── Injeksi Kegagalan Aktuator Fisik (Loss of Effectiveness) ──
  for (int i = 0; i < 4; i++) {
    if (gMotorFaultActive[i]) {
      float factor = 1.0f - (constrain(gMotorFaultPercent[i], 0.0f, 100.0f) / 100.0f);
      if (i == 0) m1 = (float)gEscMinPwm + (m1 - (float)gEscMinPwm) * factor;
      else if (i == 1) m2 = (float)gEscMinPwm + (m2 - (float)gEscMinPwm) * factor;
      else if (i == 2) m3 = (float)gEscMinPwm + (m3 - (float)gEscMinPwm) * factor;
      else if (i == 3) m4 = (float)gEscMinPwm + (m4 - (float)gEscMinPwm) * factor;
    }
  }
